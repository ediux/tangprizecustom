...
<#= include('shared/model-class-begin.cs') #>
    ...
    static <#= ProjectClassName #>()
    {
        ...
        <# if (ServerScriptExist("Global", "ContentType_Mapping")) { #>
        // ContentType Mapping event
        ContentTypeMapping(provider.Mappings);
        <# } #>

        <# if (ServerScriptExist("Global", "Class_Init")) { #>
        // Class Init event
        ClassInit();
        <# } #>
    }
    ...
    // HttpContext data
    public static HttpDataDictionary HttpData => (Container ?? RootContainer).Resolve<HttpDataDictionary>();
    ...
    public static Dictionary<string, object> GlobalClientVars()
    {
        return new () {
            ...
            { "TIMEOUT_URL", AppPath("<#= timeoutPage #>") }, // Timeout URL // DN
    ...
    /// </summary>
    /// <returns>Current Windows user name</returns>
    public static string? CurrentWindowsUser() => User?.Identity?.Name;
    <# } #>
    ...
    public static void SetupLoginStatus()
    {
        ...
        string login2FAPage = "<#= login2FAPage #>";
        <# } #>

        string logoutPage = "<#= logoutPage #>";
        ...
        <# if (isWindowsAuthenticate) { #>
        LoginStatus["canLogout"] = !Empty(logoutPage) && IsLoggedIn() && CurrentUserName() != CurrentWindowsUser();
        <# } else { #>
            <# if (useTwoFactorAuthentication) { #>
        LoginStatus["canLogout"] = !Empty(logoutPage) && (IsLoggedIn() || currentPage != login2FAPage && IsLoggingIn2FA());
            <# } else if (saml2Auth) { #>
        LoginStatus["canLogout"] = !Empty(logoutPage) && (IsLoggedIn() || IsSamlAuthenticated());
            <# } else { #>
        ...
        <# if (hasUserTable && PROJ.SecForgetPwdPage) { #>
        ...
        <# if (hasUserTable && PROJ.SecChangePwdPage) { #>
        string changePasswordPage = "<#= changePasswordPage #>";
        ...
        LoginStatus["changePasswordText"] =  Language.Phrase("ChangePassword");
            <# if (isWindowsAuthenticate) { #>
        LoginStatus["canChangePassword"] = !Empty(changePasswordUrl) && IsLoggedIn() && !IsSysAdmin() && !IsAuthenticated();
            <# } else { #>
        LoginStatus["canChangePassword"] = !Empty(changePasswordUrl) && IsLoggedIn() && !IsSysAdmin();
            <# } #>
        <# } #>

        <# if (hasUserTable && PROJ.UsePersonalData) { #>
        string personalDataPage = "<#= personalDataPage #>";
        ...
        bool isNonAdmin = IsLoggedIn() && !IsSysAdmin();
        if (isNonAdmin)
            Profile.LoadProfileFromDatabase(CurrentUserName()).GetAwaiter().GetResult();
        LoginStatus["login2FAUrl"] = GetUrl(login2FAPage);
        bool hasUserSecret = isNonAdmin && Profile.HasUserSecret(CurrentUserName(), true).GetAwaiter().GetResult();
        LoginStatus["backupCodes"] = Language.Phrase("BackupCodes");
        LoginStatus["showBackupCodes"] = isNonAdmin && hasUserSecret;
        LoginStatus["enable2FA"] = isNonAdmin && !Config.ForceTwoFactorAuthentication && !hasUserSecret;
        LoginStatus["enable2FAText"] = Language.Phrase("Enable2FA");
        LoginStatus["disable2FA"] = isNonAdmin && !Config.ForceTwoFactorAuthentication && hasUserSecret;
    ...
    protected static dynamic ResolveConnection(string dbid = "DB", string name = "")
    {
        ...
        return GetConnectionType(dbid).ToUpper() switch {
            ...
            <# } #>
            <# if (IsPostgreSQL()) { #>
            "POSTGRESQL" => Container.ResolveNamed<DatabaseConnection<NpgsqlConnection, NpgsqlCommand, NpgsqlDataReader, NpgsqlDbType>>(dbid + name, new NamedParameter("dbid", dbid)),
            <# } #>
            <# if (IsOracle()) { #>
            "ORACLE" => Container.ResolveNamed<DatabaseConnection<OracleConnection, OracleCommand, OracleDataReader, OracleDbType>>(dbid + name, new NamedParameter("dbid", dbid)),
            <# } #>
            <# if (IsMsSQL()) { #>
            ...
            <# } #>
            <# if (IsSQLite()) { #>
            "SQLITE" => Container.ResolveNamed<DatabaseConnection<SqliteConnection, SqliteCommand, SqliteDataReader, SqliteType>>(dbid + name, new NamedParameter("dbid", dbid)),
            <# } #>
            <# if (IsODBC()) { #>
            "ACCESS" => Container.ResolveNamed<DatabaseConnection<OdbcConnection, OdbcCommand, OdbcDataReader, OdbcType>>(dbid + name, new NamedParameter("dbid", dbid)),
            "ODBC" => Container.ResolveNamed<DatabaseConnection<OdbcConnection, OdbcCommand, OdbcDataReader, OdbcType>>(dbid + name, new NamedParameter("dbid", dbid)),
            <# } #>
    ...
    public static async Task WriteAuditTrailAsync(string pfx, string dt, string scrpt, string user, string action, string table, string field, object? keyvalue, object? oldvalue, object? newvalue)
    {
        ...
        try {
            Lang language = RootContainer.Resolve<Lang>();
            ...
            <# if (ServerScriptExist("Global", "AuditTrail_Inserting")) { #>
            // Call AuditTrail Inserting event
            bool writeAuditTrail = <#= Await #>AuditTrailInserting(rsnew);
            <# } else { #>
            bool writeAuditTrail = true;
            <# } #>
    ...
    /// </summary>
    public static string[] NativeDigits => NumberingSystem == "latn" ? LatinDigits : (CurrentNumberFormat?.NativeDigits ?? LatinDigits);
    ...
    public static async Task<bool> SendEmailAsync(string from, string to, string cc, string bcc, string subject, string body, string format, string charset, SecureSocketOptions options, List<Dictionary<string, string>>? attachments = null, List<string>? images = null, SmtpClient? smtp = null)
    {
        ...
        <# if (PROJ.AddPlainTextEmail) { #>
        try {
            builder.TextBody = Resolve("HtmlToText").Convert(body);
        } catch (Exception e) {}
        <# } #>
    ...
    public static string CurrentPageName() => (RouteValues != null)
        ? ConvertToString(IsApi() ? Config.ApiUrl + RouteValues["controller"] : RouteValues["action"])<# if (PROJ.OutputNameLCase) { #>.ToLowerInvariant()<# } #> // Returns action (file name) only
        : "";
    ...
    // Is directory
    public static Func<string, bool> IsDirectory { get; set; } = (path) => (File.GetAttributes(path) & System.IO.FileAttributes.Directory) == System.IO.FileAttributes.Directory;
    ...
    public static object? Invoke(string name, object?[]? parameters = null) =>
        typeof(<#= ProjectClassName #>).GetMethod(name)?.Invoke(null, parameters);
    ...
    public static object? GetPropertyValue(string name) =>
        typeof(<#= ProjectClassName #>).GetProperty(name, BindingFlags.Public | BindingFlags.Static | BindingFlags.FlattenHierarchy | BindingFlags.Instance)?.GetValue(null);
    ...
    /// Current table name
    /// </summary>
    public static string CurrentTableName() => DoesPropertyExist(CurrentTable, "TableName") ? CurrentTable?.TableName ?? "" : "";
    ...
    <#
        for (var [name, dataType] of ViewDataMap) {
            let constName = ConstantCase(name),
                resolve = InitViewData(dataType);
    #>
    // <#= CapitalCase(name) #>
    public static <#= dataType #> <#= name #>
    {
        <# if (resolve) { #>
        get => HttpData.Resolve<<#= dataType #>>("<#= constName #>");
        <# } else { #>
        get => HttpData.Get<<#= dataType #>>("<#= constName #>")!;
        <# } #>
        set => HttpData["<#= constName #>"] = value;
    }
    <#
        }
    #>
