...
<#= include('shared/model-class-begin.cs') #>
    ...
    public partial class DatabaseConnectionBase<N, M, R, T> : IDisposable
        ...
        where R : DbDataReader
    {
        ...
        public DatabaseConnectionBase(string dbid)
        {
            ...
            _queryFactory = new Lazy<QueryFactory>(() => new QueryFactory {
                ...
                Logger = compiled => logger.Log(LogLevel.Debug, compiled.ToString()), // Log the compiled query
        ...
        public async Task<R> SelectLimit(string sql, int nrows = -1, int offset = -1, bool hasOrderBy = false)
        {
            ...
            if (IsMsSql) {
                if (IsMsSql2012) { // Microsoft SQL Server >= 2012
                    ...
                    if (Regex.IsMatch(sql, @"WHERE\s+(.+?)\s+ORDER\s+BY\s+(.*).*"))
                    {
                        if (offset > -1)
        ...
        public virtual async Task<N> OpenConnectionAsync()
        {
            ...
            <#= Await #>DatabaseConnecting(ref connstr);
            ...
            if (IsOracle) {
                ...
                <# if (PROJ.OracleCompare) { #>
                await c.ExecuteAsync("ALTER SESSION SET NLS_COMP = <#= PROJ.OracleCompare #>");
                <# } #>
                <# if (PROJ.OracleSort) { #>
                await c.ExecuteAsync("ALTER SESSION SET NLS_SORT = <#= PROJ.OracleSort #>");
                <# } #>
                if (timezone != "")
        ...
        // Begin transaction
        public void BeginTrans(bool main = true)
        {
            try {
               if(main)
        ...
        <#= GetVirtualEvent("Global", "Database_Connecting") #>
        <#= GetVirtualEvent("Global", "Database_Connected") #>
    }
